...
async function loadPlans() {
  try {
    ...
    console.error(err);
    alert('Failed to load plans (check your access token)');
  }
}
...
async function loadPigeonCounts() {
  try {
    ...
    console.error(err);
    alert('Failed to load pigeon counts');
  }
}
...
async function updatePlanRow(index) {
  ...
  try {
    await gapi.client.sheets.spreadsheets.values.update({
      ...
      valueInputOption: 'USER_ENTERED',
      resource: {
        values: [[p.date, p.restaurant, p.initiator, p.participants.join(','), p.done ? '是' : '否', p.note]]
      }
    });
  } catch (err) {
    console.error(err);
...
async function deletePlan(index) {
  ...
  try {
    ...
    await gapi.client.sheets.spreadsheets.batchUpdate({
      ...
      resource: {
        requests: [{
          deleteDimension: { range: { sheetId, dimension: 'ROWS', startIndex: p.rowIndex - 1, endIndex: p.rowIndex } }
        }]
      }
    });
    await loadPlans();
...
function renderPlans() {
  ...
  plans.forEach((p, i) => {
    ...
    const tdDone = document.createElement('td'); const chk = document.createElement('input'); chk.type = 'checkbox'; chk.checked = p.done; chk.addEventListener('change', () => toggleDone(i)); tdDone.appendChild(chk);
    const tdNote = document.createElement('td'); const noteInput = document.createElement('input'); noteInput.type = 'text'; noteInput.value = p.note; noteInput.addEventListener('change', () => updateNote(i, noteInput.value)); tdNote.appendChild(noteInput);
    const tdOp = document.createElement('td'); const delBtn = document.createElement('button'); delBtn.innerHTML = '<i class="fas fa-trash-alt"></i> Delete'; delBtn.addEventListener('click', () => deletePlan(i)); tdOp.appendChild(delBtn);
...
function renderRank() {
  ...
  Object.entries(pigeonCounts).sort((a, b) => b[1] - a[1]).forEach(([name, count]) => {
    ...
    const tdBtn = document.createElement('td'); const btn = document.createElement('button'); btn.textContent = '放鸽子 +1'; btn.addEventListener('click', () => incrementPigeon(name)); tdBtn.appendChild(btn);
